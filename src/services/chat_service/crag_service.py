...
class CRAGService:
    ...
    def generate(
        ...
        if verbose:
            logger.info(f"1  Initial retrieval (k={self.initial_k})...")
        ...
        if confidence_initial >= confidence_threshold:
            ...
            if verbose:
                logger.warning(f"     Low confidence - applying corrective retrieval...\n")
            
            # Step 3: Corrective retrieval
            if verbose:
                logger.info(f"2 Corrective retrieval (k={self.expanded_k}, expanded)...")
