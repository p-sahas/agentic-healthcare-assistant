...
class LLMDataGenerator(DataGenerator):
    ...
    def generate_doctors(self, n: int, specialties: List[str]) -> List[Dict]:
        ...
        self.logger.info(
            f"🤖 Generating {n} Sri Lankan doctor names via LLM...")
...
Generate exactly {n} doctors:"""
    ...
    def generate_patients(self, n: int) -> List[Dict]:
        ...
        self.logger.info(
            f"🤖 Generating {n} Sri Lankan patient names via LLM...")
...
class TemplateDataGenerator(DataGenerator):
    ...
    def generate_patients(self, n: int) -> List[Dict]:
        ...
        self.logger.info(
            f"✓ Generated {len(patients)} patients from templates")
        return patients
    ...
    def _clear_existing_data(self):
        ...
        try:
            ...
            if result.fetchone() is None:
                self.logger.info(
                    "⏭️  Tables don't exist yet, skipping data clear")
                return
        except Exception:
...
class JSONLStorageAdapter(StorageAdapter):
    ...
    def finalize(self):
        ...
        self.logger.info(
            f"✓ Wrote {len(self.bookings)} bookings to {self.config.output_file}")
...
class UnifiedCRMSeeder:
    ...
    LOCATIONS = [
        {"name": "Nawaloka Hospitals", "type": "HOSPITAL",
            "address": "23 Deshamanya H K Dharmadasa Mawatha, Colombo 00200"},
        {"name": "Nawaloka City OPD", "type": "OPD",
            "address": "123 Galle Road, Colombo 03"},
        {"name": "Central Lab", "type": "LAB",
            "address": "45 Baseline Road, Colombo 09"},
        {"name": "Heart Care Clinic", "type": "CLINIC",
    ...
    def _seed_from_sql(self) -> bool:
        ...
        self.logger.info(
            "📂 Found pre-exported SQL seed files - loading deterministic data")
        ...
        try:
            with engine.connect() as conn:
                for sql_file in self.SQL_SEED_FILES:
                    ...
                    conn.commit()
                    # "01_specialties" → "specialties"
                    table_name = path.stem.split("_", 1)[1]
                    self.logger.info(
                        f"  ✅ {table_name}: {row_count} rows loaded from {sql_file}")
    ...
    def seed(self):
        ...
        if self.config.storage_backend == StorageBackend.DATABASE and self._sql_files_exist():
            ...
            if self._seed_from_sql():
                ...
                self.logger.info("=" * 70)
                self.logger.info(
                    f"✅ CRM seeded from SQL files in {elapsed:.1f}s")
                self.logger.info("=" * 70)
                return

        #  Fallback: generate data dynamically
        self.logger.info(
            "⚙️  No SQL seed files found - generating data dynamically")
        ...
        # Seed bookings
        bookings_count = self._seed_bookings(
            doctors, patients, locations, specialties)
    ...
    def _seed_specialties(self) -> List:
        ...
        self.logger.info(
            f"🏥 Seeding {len(specialties_to_create)} specialties...")
    ...
    def _seed_doctors(self, specialties: List) -> List:
        ...
        doctors_data = self.generator.generate_doctors(
            self.config.n_doctors, specialty_names)
        ...
        for i, doc_data in enumerate(doctors_data):
            ...
            self.storage.store_data(data)
            # Update with prefix for return value
            doc_data['full_name'] = full_name
            doctors.append({'id': doctor_id, **doc_data,
                           'specialty_id': specialty_map.get(doc_data['specialty'])})
    ...
    def _seed_patients(self) -> List:
        ...
        patients_data = self.generator.generate_patients(
            self.config.n_patients)
        medical_notes = self.generator.generate_medical_notes(
            self.config.n_patients)
        ...
        for i, patient_data in enumerate(patients_data):
            ...
            self.storage.store_data(data)
            patients.append({'id': patient_id, **patient_data,
    ...
    def _seed_bookings(self, doctors: List, patients: List, locations: List, specialties: List) -> int:
        """Seed bookings."""
        self.logger.info(
            f"📅 Seeding bookings ({self.config.n_slots_per_doctor} slots per doctor)...")
        ...
        for specialty in specialties:
            reasons = self.generator.generate_appointment_reasons(
                20, specialty['name'])
        ...
        for doctor_idx, doctor in enumerate(doctors):
            ...
            # Group every 5 doctors per day
            current_date = start_dt + timedelta(days=doctor_idx // 5)
            ...
            for slot_dt in doctor_slots[:self.config.n_slots_per_doctor]:
                ...
                reasons = specialty_reasons.get(
                    specialty_name, ["Consultation"])
...
Examples:
  ...
  python seed_crm_unified.py --n-doctors 100 --n-patients 500
    ...
    # Scale parameters
    parser.add_argument('--n-doctors', type=int,
                        default=10, help='Number of doctors')
    parser.add_argument('--n-patients', type=int,
                        default=20, help='Number of patients')
    parser.add_argument('--n-specialties', type=int,
                        default=7, help='Number of specialties')
    parser.add_argument('--n-locations', type=int,
                        default=4, help='Number of locations')
    parser.add_argument('--n-slots-per-doctor', type=int,
                        default=15, help='Slots per doctor')

    # Scheduling
    parser.add_argument(
        '--start', default=datetime.now().strftime("%Y-%m-%d"), help='Start date')
